 ...
 * and then get out of the way: the primary action is starting a match, not
 * signing up, because scoring works without an account and asking for a phone
 * number before showing value is how first-run flows lose people.
 */
  ...
  return (
    <SafeAreaView className="bg-background flex-1">
      ...
      <ScrollView contentContainerClassName="px-5 pb-6 pt-6 grow">
        ...
        <View className="mt-8">
          <Button label="Start a match" onPress={() => router.push('/phone')} />
          <View className="mt-3 items-center">
            <Text className="text-foreground/60 font-heading text-[11px] uppercase tracking-[1.4px]">
              Already scoring?{' '}
              <Text className="text-steel-700" onPress={() => router.push('/phone')}>
                Sign in
              </Text>
            </Text>
          </View>
