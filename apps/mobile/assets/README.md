...
From the Industry system. Two colours only, and both are literals here on
purpose: the app resolves its palette from CSS variables in `global.css` at
runtime, and a PNG generated by a Python script cannot read those. Keep them
in step by hand if the palette ever moves.
